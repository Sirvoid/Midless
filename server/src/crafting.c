...
static ItemStack ReadIngredient(int index, bool *exact) {
    ...
    if (lua_istable(L, index)) {
        lua_getfield(L, index, "id"); stack.itemId = ServerItems_Id(L, -1, false, false); lua_pop(L, 1);
        lua_getfield(L, index, "metadata"); *exact = !lua_isnil(L, -1); lua_pop(L, 1);
        LuaMetadata_ReadItem(L, index, &stack);
    } else stack.itemId = ServerItems_Id(L, index, false, false);
    stack.count = stack.itemId ? 1 : 0;
    return stack;
}
static bool Matches(const Recipe *recipe, int ingredient, ItemStack stack) {
    return recipe->ingredients[ingredient].itemId == stack.itemId &&
        (!recipe->exact[ingredient] || ItemStack_Matches(recipe->ingredients[ingredient], stack));
}
int Crafting_Register(void) {
    ...
    ServerItems_ReadStack(L, -1, &recipe.output); lua_pop(L, 1);
    ...
    if (recipe.shapeless) {
        ...
        for (int i = 0; i < count; i++) {
            lua_rawgeti(L, -1, i + 1); recipe.ingredients[i] = ReadIngredient(-1, &recipe.exact[i]); lua_pop(L, 1);
            if (!recipe.ingredients[i].itemId) return luaL_error(L, "shapeless ingredients cannot be empty");
        }
    } else {
        lua_pop(L, 1);
        luaL_checktype(L, -1, LUA_TTABLE);
        ...
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                lua_rawgeti(L, -1, x + 1); ItemStack ingredient = ReadIngredient(-1, &exact[y * 3 + x]); lua_pop(L, 1);
    ...
    *match = (CraftingMatch){0};
    ...
    for (int r = 0; r < recipeCount; r++) {
        ...
        if (recipe->shapeless) {
            ...
            for (int pass = 0; pass < 2 && ok; pass++) for (int i = 0; i < recipe->count && ok; i++) {
                if (recipe->exact[i] != (pass == 0)) continue;
                int j;
                for (j = 0; j < rows * columns; j++) if (!used[j] && slots[j].count && Matches(recipe, i, slots[j])) {
                    used[j] = true; break;
                }
                if (j == rows * columns) ok = false;
            }
        } else {
            ...
            for (int y = 0; y < recipe->height && ok; y++) for (int x = 0; x < recipe->width; x++) {
                ...
                if (!Matches(recipe, y * recipe->width + x, *slot)) { ok = false; break; }
            }
        }
        if (!ok) continue;
        match->recipe = r;
