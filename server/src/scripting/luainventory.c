...
static int Integer(lua_State *state, int index, int low, int high) {
    ...
    if (n < low || n > high)
        luaL_error(state, "inventory value out of range");
    return n;
}
static ItemStack *Slots(lua_State *state, int index, int *count) {
    ...
    if (!inventory)
        luaL_error(state, "player inventory is undefined or its saved size differs");
    *count = inventory->count;
    return inventory->slots;
}
static int GetStack(lua_State *state) {
    int count;
    ItemStack *slots = Slots(state, 1, &count);
    ItemStack stack = slots[Integer(state, 2, 1, count) - 1];
    LuaItems_PushStack(state, stack);
    return 1;
}
static int SetStack(lua_State *state) {
    ...
    if (!lua_isnoneornil(state, 3))
        LuaItems_ReadStack(state, 3, &stack);
    slots[slot] = stack;
    p->inventoryRevision++;
...
static int AddItem(lua_State *state) {
    ...
    LuaMetadata_ReadItem(state, 2, &stack);
    int slotCount;
    ItemStack *slots = Slots(state, 1, &slotCount);
    ItemStack next[255];
    memcpy(next, slots, slotCount * sizeof(ItemStack));
    bool added = Inventory_AddStackToSlots(
                     next, slotCount, slots == p->inventory.slots ? INVENTORY_STORAGE_SLOTS : 0,
                     stack, count) == count;
    if (added) {
...
int LuaInventory_Get(lua_State *state, Player *p) {
    ...
    if (!lua_isnoneornil(state, 2)) {
        ...
        if (!length || length > 64 || memchr(value, 0, length))
            return luaL_error(state, "invalid inventory name");
        memcpy(name, value, length);
        if (!PlayerInventories_Get(p, name))
            return luaL_error(state, "player inventory is undefined or its saved size differs");
    }
...
static void Text(lua_State *state, int table, const char *key, char value[65]) {
    ...
    if (length > 64 || memchr(s, 0, length))
        luaL_error(state, "UI text supports up to 64 bytes without NUL");
    memcpy(value, s, length);
    value[length] = 0;
    lua_pop(state, 1);
}
static float Number(lua_State *state, int table, const char *key) {
    ...
    if (!isfinite(n) || n < 0 || n > 32)
        luaL_error(state, "UI dimensions must be between 0 and 32");
    lua_pop(state, 1);
    return n;
}
static int Bind(lua_State *state, Player *p, InventoryWindow *window, int index) {
    ...
    int slots;
    if (!block) {
        if (Owner(state, index) != p)
            return luaL_error(state, "screen must use this player's inventories");
        PlayerInventory *handle = luaL_checkudata(state, index, PLAYER_INVENTORY);
        ...
        if (!window->block)
            return luaL_error(state, "block inventories require layout.block");
    ...
    for (int i = 1; i < view->bindingCount; i++) {
        if (window->bindings[i].block == block && !strcmp(window->bindings[i].name, name))
            return i;
        if (block && window->bindings[i].block)
            return luaL_error(state, "a screen supports one block inventory");
    }
    if (view->bindingCount == INVENTORY_VIEW_BINDINGS ||
        view->slotCount + slots > INVENTORY_VIEW_SLOTS)
...
int LuaInventory_Show(lua_State *state, Player *p) {
    ...
    v->bindingCount = 1;
    v->bindingSlots[0] = INVENTORY_SLOT_COUNT;
    Text(state, 2, "title", v->title);
    v->width = Number(state, 2, "width");
    v->height = Number(state, 2, "height");
    ...
    if (!count || count > INVENTORY_VIEW_ELEMENTS)
        return luaL_error(state, "inventory UI supports 1..16 elements");
    v->count = count;
    for (int i = 0; i < v->count; i++) {
        ...
        e->x = Number(state, element, "x");
        e->y = Number(state, element, "y");
        lua_getfield(state, element, "type");
        const char *type = luaL_checkstring(state, -1);
        bool label = !strcmp(type, "label");
        e->grid = !strcmp(type, "inventory");
        e->crafting = !strcmp(type, "crafting_output");
        e->progress = !strcmp(type, "progress");
        lua_pop(state, 1);
        if (label)
            Text(state, element, "text", e->text);
        else if (e->progress) {
            e->width = Number(state, element, "width");
            e->height = Number(state, element, "height");
            lua_getfield(state, element, "max");
            e->maximum = luaL_checknumber(state, -1);
            lua_pop(state, 1);
            lua_getfield(state, element, "value");
            if (lua_type(state, -1) == LUA_TSTRING) {
                if (!window.block)
                    return luaL_error(state, "metadata progress requires layout.block");
                Text(state, element, "value", window.progressFields[i]);
                if (!ServerMetadata_Progress(window.position, window.progressFields[i], &e->value))
                    return luaL_error(state, "progress requires numeric block metadata");
            } else
                e->value = luaL_checknumber(state, -1);
            lua_pop(state, 1);
        } else if (e->grid || e->crafting) {
            lua_getfield(state, element, "slot");
            e->first = lua_isnil(state, -1) ? 0 : Integer(state, -1, 1, 255) - 1;
            lua_pop(state, 1);
            lua_getfield(state, element, "columns");
            e->columns = Integer(state, -1, 1, 32);
            lua_pop(state, 1);
            lua_getfield(state, element, "rows");
            e->rows = Integer(state, -1, 1, 32);
            lua_pop(state, 1);
            ...
            if (e->crafting) {
                ...
                if (!window.recipes[i][0])
                    return luaL_error(state, "recipe group cannot be empty");
            }
        } else
            return luaL_error(state, "unknown inventory UI element type");
        lua_pop(state, 1);
    }
    if (!InventoryView_Validate(v))
        return luaL_error(state, "screen grids must cover each inventory slot once, without "
                                 "overlapping, within the layout");
...
void LuaInventory_Init(void) {
    ...
    const luaL_Reg methods[] = {
        {"get_stack", GetStack}, {"set_stack", SetStack}, {"add_item", AddItem}, {NULL, NULL}};
    luaL_newmetatable(L, PLAYER_INVENTORY);
    lua_newtable(L);
    luaL_setfuncs(L, methods, 0);
    lua_setfield(L, -2, "__index");
    lua_pushliteral(L, PLAYER_INVENTORY);
...
int LuaInventory_Define(lua_State *state) {
    ...
    if (!length || length > 64 || memchr(name, 0, length))
        return luaL_error(L, "invalid inventory name");
    luaL_checktype(L, 2, LUA_TTABLE);
    lua_getfield(L, 2, "slots");
    int count = Integer(L, -1, 1, 255);
    if (!PlayerInventories_Define(name, count))
...
int LuaInventory_DefineScreen(lua_State *state) {
    ...
    lua_pushvalue(L, 1);
    screenCallback = luaL_ref(L, LUA_REGISTRYINDEX);
    return 0;
}
static int BuildPlayerScreen(lua_State *state) {
    ...
    if (!lua_isnil(state, -1))
        return luaL_error(state, "the player inventory screen cannot depend on a block");
    lua_pop(state, 1);
    return LuaInventory_Show(state, p);
}
bool ScriptHooks_InventoryOpenPlayer(Player *p) {
    if (screenCallback == LUA_NOREF)
        return false;
    int top = lua_gettop(L);
    lua_pushcfunction(L, BuildPlayerScreen);
    lua_pushlightuserdata(L, p);
    bool ok = lua_pcall(L, 1, 1, 0) == LUA_OK;
    if (!ok)
        TraceLog(LOG_WARNING, "Player inventory screen: %s", lua_tostring(L, -1));
    else
