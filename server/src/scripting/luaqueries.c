...
static Vector3 ReadPosition(int index) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (!isfinite(value) || fabs(value)>999900) luaL_error(L,"query position is outside the supported range");
        values[i] = value; lua_pop(L,1);
    }
...
static void PushHit(WorldHit hit) {
    ...
    if (hit.type == HIT_BLOCK) {
        type = "block"; LuaMetadata_PushBlock(L,hit.block); lua_setfield(L,-2,"block");
    } else if (hit.type == HIT_UNLOADED) type = "unloaded";
        ...
        if (e->ownerPlayerId >= 0) {
            ...
            if (player && !player->disconnected) {
                type = "player"; LuaBindings_PushPlayer(player); lua_setfield(L,-2,"player");
            }
        } else { type = "entity"; LuaEntities_Push(e); lua_setfield(L,-2,"entity"); }
    }
    lua_pushstring(L,type); lua_setfield(L,-2,"type");
    PushPosition(hit.position); lua_setfield(L,-2,"position");
    PushPosition(hit.normal); lua_setfield(L,-2,"normal");
...
int LuaQueries_Raycast(void) {
    ...
    if (hypotf(hypotf(to.x-from.x,to.z-from.z),to.y-from.y)>128) return luaL_error(L,"raycast is limited to 128 blocks");
    bool entities = true; int ignore = -1;
    if (!lua_isnoneornil(L,3)) {
        luaL_checktype(L,3,LUA_TTABLE);
        lua_getfield(L,3,"entities");
        if (!lua_isnil(L,-1)) { luaL_checktype(L,-1,LUA_TBOOLEAN); entities = lua_toboolean(L,-1); }
        lua_pop(L,1);
        lua_getfield(L,3,"ignore");
        if (!lua_isnil(L,-1)) ignore = LuaEntities_Check(L,-1)->id;
        lua_pop(L,1);
        lua_getfield(L,3,"ignore_player");
        if (!lua_isnil(L,-1)) {
            lua_Integer id = luaL_checkinteger(L,-1);
            if (id < 0 || id >= WORLD_MAX_PLAYERS) return luaL_error(L,"invalid player id");
            if (serverWorld.players && serverWorld.players[id]) ignore = serverWorld.players[id]->entityId;
        }
        lua_pop(L,1);
    }
    PushHit(ServerQuery_Raycast(from,to,entities,ignore)); return 1;
}
static int Nearby(bool players) {
    Vector3 p = ReadPosition(1);
    double radius = luaL_checknumber(L,2);
    if (!isfinite(radius) || radius<0 || radius>128) return luaL_error(L,"query radius must be 0..128");
    BoundingBox area = {{p.x-radius,p.y-radius,p.z-radius},{p.x+radius,p.y+radius,p.z+radius}};
    int ids[WORLD_MAX_ENTITIES], count = ServerPhysics_QueryEntities(area,ids,WORLD_MAX_ENTITIES), added = 0;
    lua_newtable(L);
    for (int i = 0; i < count; i++) {
        ...
        if (players) {
            if (e->ownerPlayerId<0 || !serverWorld.players) continue;
            Player *player = serverWorld.players[e->ownerPlayerId];
            if (!player || player->disconnected || !player->movementReady) continue;
            LuaBindings_PushPlayer(player);
        } else {
            if (e->ownerPlayerId>=0 || e->definitionId<0) continue;
            LuaEntities_Push(e);
        }
...
int LuaQueries_NearestPlayer(void) {
    ...
    for (int i=0; serverWorld.players && i<WORLD_MAX_PLAYERS; i++) {
        ...
        if (!p || p->disconnected || !p->movementReady || p->entityId<0 || p->entityId>=WORLD_MAX_ENTITIES) continue;
        ...
        if (distance>best || (nearest && distance==best)) continue;
        LuaBindings_PushPlayer(p); lua_getfield(L,-1,"get_hp"); lua_pushvalue(L,-2); lua_call(L,1,1);
        bool living = lua_tointeger(L,-1)>0; lua_pop(L,2);
        if (living) { nearest = p; best = distance; }
    }
    LuaBindings_PushPlayer(nearest); return 1;
}
int LuaQueries_FindPath(void) {
    ...
    for (int i = 0; i < count; i++) { PushPosition(path[i]); lua_rawseti(L,-2,i+1); }
    return 1;
}
...
int LuaQueries_RegisterAttack(void) {
    ...
    if (attackCount == 64) return luaL_error(L,"too many attack callbacks");
    lua_pushvalue(L,1); attackCallbacks[attackCount++] = luaL_ref(L,LUA_REGISTRYINDEX);
    return 0;
}
void LuaQueries_Attack(Player *player) {
    double now = GetTime();
    if (!attackCount || !player->movementReady || now<player->nextMeleeAttack || player->entityId<0) return;
    player->nextMeleeAttack = now+0.4;
    Entity *e = &serverWorld.entities[player->entityId];
    Vector3 from = e->position; from.y += 1.5f;
    float yaw = e->rotation.y, pitch = e->rotation.x, horizontal = cosf(pitch);
    Vector3 to = {from.x+sinf(yaw)*horizontal*4.5f,from.y-sinf(pitch)*4.5f,from.z+cosf(yaw)*horizontal*4.5f};
    WorldHit hit = ServerQuery_Raycast(from,to,true,e->id);
    uint64_t generation = hit.type == HIT_ENTITY ? serverWorld.entities[hit.entityId].generation : 0;
    int count = attackCount, top = lua_gettop(L);
    for (int i = 0; i < count; i++) {
        if (hit.type == HIT_ENTITY) {
            Entity *target = &serverWorld.entities[hit.entityId];
            if (!target->active || target->pendingRemoval || target->generation != generation) break;
        }
        lua_rawgeti(L,LUA_REGISTRYINDEX,attackCallbacks[i]);
        LuaBindings_PushPlayer(player); PushHit(hit);
        bool handled = false;
        if (lua_pcall(L,2,1,0) != LUA_OK) TraceLog(LOG_WARNING,"Player attack: %s",lua_tostring(L,-1));
...
void LuaQueries_Reset(void) {
    for (int i = 0; i < attackCount; i++) luaL_unref(L,LUA_REGISTRYINDEX,attackCallbacks[i]);
    attackCount = 0;
}
