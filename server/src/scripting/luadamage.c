...
Vector3 LuaDamage_Impulse(lua_State *state, int context, Entity *target) {
    ...
    if (!lua_isnil(state, -1)) {
        ...
        else if (!luaL_testudata(state, -1, "midless.Entity") &&
                 !luaL_testudata(state, -1, "midless.Player"))
    ...
    if (!lua_isnil(state, -1)) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (!isfinite(n) || fabs(n) > 1000000)
                luaL_error(state, "invalid damage direction");
            values[i] = n;
            lua_pop(state, 1);
        }
    ...
    if (!lua_isnil(state, -1)) {
        luaL_checktype(state, -1, LUA_TTABLE);
        const char *names[] = {"horizontal", "upward"};
        float *values[] = {&horizontal, &upward};
        for (int i = 0; i < 2; i++) {
            lua_getfield(state, -1, names[i]);
            double n = luaL_optnumber(state, -1, 0);
            if (!isfinite(n) || n < 0 || n > 20)
                luaL_error(state, "damage knockback must be 0..20");
            *values[i] = n;
            lua_pop(state, 1);
        }
    }
...
int LuaDamage_Result(lua_State *state, int index, int amount) {
    if (lua_isnil(state, index))
        return amount;
    if (lua_isboolean(state, index) && !lua_toboolean(state, index))
        return 0;
    if (lua_isinteger(state, index) && lua_tointeger(state, index) >= 0 &&
        lua_tointeger(state, index) <= 65535)
...
int LuaDamage_RegisterPlayer(void) {
    ...
    lua_pushvalue(L, 1);
    playerCallbacks[playerCallbackCount++] = luaL_ref(L, LUA_REGISTRYINDEX);
    return 0;
}
int LuaDamage_PlayerHooks(Entity *target, int context, int amount) {
    ...
    for (int i = 0; i < count && amount > 0; i++) {
        ...
        if (lua_pcall(L, 3, 1, 0) != LUA_OK) {
            TraceLog(LOG_WARNING, "Player on_damage: %s", lua_tostring(L, -1));
            amount = 0;
        } else
            amount = LuaDamage_Result(L, -1, amount);
        lua_pop(L, 1);
    }
    return amount;
}
void LuaDamage_Reset(void) {
    for (int i = 0; i < playerCallbackCount; i++)
        luaL_unref(L, LUA_REGISTRYINDEX, playerCallbacks[i]);
