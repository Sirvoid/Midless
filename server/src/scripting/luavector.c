...
static double Number(lua_State *state, int index) {
    ...
    if (!isfinite(value))
        luaL_argerror(state, index, "expected a finite number");
    return value;
}
...
static int PushNumber(lua_State *state, double value) {
    if (!isfinite(value))
        return luaL_error(state, "vector calculation overflowed");
...
static int Push(lua_State *state, LuaVector v) {
    ...
    PushNumber(state, v.x);
    lua_setfield(state, -2, "x");
    PushNumber(state, v.y);
    lua_setfield(state, -2, "y");
    PushNumber(state, v.z);
...
static double Magnitude(LuaVector v) {
...
static LuaVector Normalized(lua_State *state, LuaVector v) {
    ...
    if (!isfinite(scale))
        luaL_error(state, "vector calculation overflowed");
    if (scale == 0)
        return (LuaVector){0};
...
static int New(lua_State *state) {
    if (lua_gettop(state) == 0)
        return Push(state, (LuaVector){0});
    if (lua_istable(state, 1))
        return Push(state, Read(state, 1));
...
static int Length(lua_State *state) {
    return PushNumber(state, Magnitude(Read(state, 1)));
}
...
static int Normalize(lua_State *state) {
    return Push(state, Normalized(state, Read(state, 1)));
}
...
static int Cross(lua_State *state) {
    LuaVector a = Read(state, 1), b = Read(state, 2);
    return Push(state,
                (LuaVector){a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x});
}

void LuaVector_Init(void) {
    static const luaL_Reg methods[] = {{"new", New},
                                       {"add", Add},
                                       {"subtract", Subtract},
                                       {"multiply", Multiply},
                                       {"length", Length},
