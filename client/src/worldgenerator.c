...
int WorldGenerator_Generate(Chunk *chunk, Vector3 pos, int index) {
    ...
    float nsCave = fnlGetNoise3D(&noiseCave, pos.x * 2.5f, (pos.y + seed) * 2.5f, pos.z * 2.5f);
    float nsCave2 = fnlGetNoise3D(&noiseCave, pos.x * 2.5f, (pos.y + 2048 + seed) * 2.5f, pos.z * 2.5f);

    float biomeElevation = (fnlGetNoise2D(&heightNoise, pos.x * 0.05f, pos.z * 0.05f) + 1.0f);

    if(pos.y >= 32 * biomeElevation) {
        float nsOff = fnlGetNoise3D(&offsetNoise, pos.x * 2.0f, (pos.y + seed) * 2.0f, pos.z * 2.0f) * 12.0f * (biomeElevation - 1);
        float ns = (fnlGetNoise3D(&noise, (pos.x + nsOff) * 2.5f, seed + nsOff, (pos.z + nsOff) * 2.5f) * nsOff + 1.0f) / 2.0f * 64 * biomeElevation;
        float ny = pos.y - 32;
        
        float waterLevel = 58 + biomeElevation * 2;
        ...
        if( ny < ns &&  fabs(nsCave) + fabs(nsCave2) < 0.3f) blockID = 0; //Caves
    } else {
        blockID = 1;
        if(fabs(nsCave) + fabs(nsCave2) < 0.3f) blockID = 0; //Caves
    }
